    ...
    int *ipiv)
{
	...
	if (schur_size > 0) {
		...
		ret = clapack_dgetrs(order, CblasNoTrans, elim_size, schur_size,
		    matrix_a, lda, ipiv, matrix_b, lda);
		if (ret != 0)
			return (ret);
...
main()
{
	...
	ret = compute_schur(CblasColMajor, matrix_a, matrix_b, matrix_c,
	    matrix_d, elim_size, schur_size, lda, ipiv1);
	...
	ret = solve(CblasColMajor, matrix_d, NULL, NULL, NULL, rhs + nrhs *
	    elim_size, schur_size, 0, nrhs, lda, ipiv2);
	assert(ret == 0);
